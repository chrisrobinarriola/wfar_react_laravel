...
class Edit_Semester extends Component
{
    state = {
        name: '',
        weeks: [],
    ...
    updateSemester = async (e) => {
        ...
        const res = await axios.put(`http://127.0.0.1:8000/api/update-semester/${stud_id}`, this.state);
        ...
        if(this.state.weeks.length >= 0){
                ...
                this.state.weeks.map((item) => {
                    return (
                        <tr key={item.id}>
                            ...
                            <td>{item.name}</td>
                            <td>
                                <Link to={`/edit-week/${item.id}/${this.props.params.id}`} className="btn btn-dark btn-sm">View</Link>
                            </td>
                        </tr>
                    );
                });
            ...
            if(this.state.loading){
                ...
                <tr key={this.state.weeks.id}>
                    ...
                    <td>
                        <Link to={`/edit-week/${this.state.weeks.id}`} className="btn btn-success btn-sm">View</Link>
                    </td>
                </tr>
            }
        }
                    ...
                    <div className="col-md-12">
                        <div className="card">
                            <div className="card-header">
                                <h4>Edit Semester
                                    <Link to={'/semester'} className="btn btn-warning btn-sm float-end"> BACK </Link>
                                </h4>
                            </div>
                            ...
                            <div className="card-header">
                                <h4>Weeks on this semester
                                    <Link to={'/add-week/' + this.props.params.id} className="btn btn-light btn-sm float-end"> Add Week </Link>
...
export default withParams(Edit_Semester);